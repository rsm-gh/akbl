...
def GET_last_configuration():
    ...
    for num, key in enumerate(sorted(profiles.keys())):
        ...
        if max==None or profile.time > max:
            max=profile.time
            profile_numb=num
...
class New:
    ...
    def create_profile(self, name, path, speed=False):
        ...
        for area in self.computer.regions.values():
            self.add_area(area)
            zone_data=ZoneData(area.regionId, self.computer.default_mode, area, self.computer.default_color, self.computer.default_color)
    ...
    def load(self, path):
        ...
        # once everything has been imported, if something is missing the default conf will be added.
        added_areas=[]
            ...
            if line.strip():
                ...
                if len(variables) == 2:
                    ...
                    if current_area and color1 and color2 and mode:                 
                        self.add_zone(ZoneData(current_area.regionId, mode, current_area, color1, color2))
